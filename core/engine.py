...
class ArticleEngine:
    ...
    def analyze(self, pdf_file, journal_name=None):
        ...
        # Journal block is optional
        journal_block = ""
        if journal_name and journal_name.strip():
            journal_block = f"""
        5. JOURNAL COMPATIBILITY: Evaluate if this article is a good fit for the journal "{journal_name}".
           Consider: scope match, rigor level, typical audience, and publication standards.
           Add a field "journal_analysis" to the JSON with:
           - "journal_name": "{journal_name}"
           - "compatibility_score": integer 0-10
           - "compatibility_verdict": one of: "EXCELENTE FIT" / "BOM FIT" / "FIT MARGINAL" / "INCOMPATÍVEL"
           - "compatibility_justificativa": brief explanation in Portuguese (2-3 sentences)
        """
        ...
        {{
            ...
            {', "journal_analysis": {{ "journal_name": "", "compatibility_score": 0, "compatibility_verdict": "", "compatibility_justificativa": "" }}' if journal_name and journal_name.strip() else ''}
        }}
        ...
        for modelo in modelos_para_testar:
            try:
                ...
                # Sanitize journal_analysis scores if present
                if "journal_analysis" in dados:
                    ja = dados["journal_analysis"]
                    try:
                        ja["compatibility_score"] = max(0, min(10, int(float(ja.get("compatibility_score", 0)))))
                    except:
                        ja["compatibility_score"] = 0
